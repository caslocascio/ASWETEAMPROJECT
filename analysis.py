...
def check_aged_reviews(reviews, min_date):
    ...
    df = pd.DataFrame(reviews, columns=['professor', 'class', 'date', 'review', 'workload', 'agree', 'disagree', 'funny'])
    # convert data types
    df['date'] = pd.to_datetime(df['date'], format='%B %d, %Y')
    # compare dates and count reviews that older than the min_date
    n_old_reviews = df[df['date']<=datetime.strptime(min_date, '%Y-%m-%d')].shape[0]
    # compare dates and count reviews that newer than the min_date
    n_new_reviews = df[df['date']>datetime.strptime(min_date, '%Y-%m-%d')].shape[0]

    return n_old_reviews, n_new_reviews


# calculate the sentiment score
def review_sentiment_analysis(df, polarity_threshold=0.1, subjectivity_threshold=0.5):
    """
    ...
    # word stemming
    st = PorterStemmer()
    df['review'] = df['review'].apply(lambda x: ' '.join([st.stem(word) for word in str(x).split()]))
    # TextBlob returns two scores for sentiment analysis - polarity and subjectivity
    ...
    df['senti_fl'] = 'neutral'
    df.loc[df['polarity_score'] < -polarity_threshold, 'senti_fl'] = 'negative'
    df.loc[df['polarity_score'] > polarity_threshold, 'senti_fl'] = 'positive'
    # decide if the review is objective or subjective based on the threshold
    df['sub_fl'] = 'objective'
    df.loc[df['subjectivity_score'] > subjectivity_threshold, 'sub_fl'] = 'subjective'
    # return counts
    return df, df[df['senti_fl']=='positive'].shape[0], df[df['senti_fl']=='neutral'].shape[0],\
            df[df['senti_fl']=='negative'].shape[0], df[df['sub_fl']=='objective'].shape[0],\
